...
object HandfulOfOlivesTest extends SimpleIOSuite {
  ...
  pureTest("There are three olives in a handful of three olives") {
    expect(countHandful(Toppings.grabHandfulOfOlives(3)) == 3)
  }
  ...
  pureTest("Toppings.toNicoise exchanges olives with the same number of Nicoise olives") {
    expect(countHandful(Toppings.toNicoise(Toppings.grabHandfulOfOlives(2))) == 2)
      .and(expect(countHandful(Toppings.toNicoise(Toppings.grabHandfulOfOlives(5))) == 5))
      .and(expect(countHandful(Toppings.toNicoise(Toppings.grabHandfulOfOlives(0))) == 0))
  }

  pureTest("Toppings.stuffWithPimento exchanges olives with the same number of Nicoise olives") {
    expect(countHandful(Toppings.stuffWithPimento(Toppings.grabHandfulOfOlives(2))) == 2)
      .and(expect(countHandful(Toppings.stuffWithPimento(Toppings.grabHandfulOfOlives(5))) == 5))
      .and(expect(countHandful(Toppings.stuffWithPimento(Toppings.grabHandfulOfOlives(0))) == 0))
  }
